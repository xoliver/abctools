...
def find_chords(bar, chords):
    ...
    for (accidentals, note, duration) in notes:
        ...
        duration = 1 if len(duration) == 0 \
            else calculate_note_duration(duration)
...
def get_top_chords(found_chords, mode_bias=None, root_bias=None):
    ...
    if len(top) == 0:
        print '*WARNING* Biases removed all the options for this bar,'\
              'using no biases here'
        top = original_top
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate chord list for each bar of an ABC tune file')
    parser.add_argument(
        'fname', metavar='FILE.abc', type=str, help='file name')
    parser.add_argument(
        '-d', '--dim',
        action='store_true', help='incorporate diminished chord')
    parser.add_argument(
        '-m', '--bias_own_mode',
        action='store_true',
        help='bias own mode (major or minor) when chords are tied')
    parser.add_argument(
        '-r', '--bias_own_root',
        action='store_true', help='bias root of the key when chords are tied')
    parser.add_argument(
        '-v', '--verbose',
    ...
    scale_chords = generate_chord_dict(
        key[0], major=is_major_key, skip_dim=not args.dim)
    if args.verbose:
    ...
    for bar in extract_bars(abc['tune']):
        ...
        if args.bias_own_root:
            ...
            rbias = None
        print i, bar, get_top_chords(
            found_chords, mode_bias=mbias, root_bias=rbias)
